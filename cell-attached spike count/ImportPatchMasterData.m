function DataChannels = ImportPatchMasterData(thisfile)
% Xiang Liao (PhD)
% Importing HEKA PatchMaster DAT files, modified from ImportHEKA, sigTOOL
% DataChannels = ImportPatchMasterData('E:\matlab\Membrane\wm\wm130729_2.dat');
...
for grp=1:numel(grp_row)    
    DataOutput=LocalImportGroup(fh, tree, grp, grp_row, channelnumber);
    DataChannels=[DataChannels,DataOutput];
end
...
%--------------------------------------------------------------------------
function DataChannels=LocalImportGroup(fh, tree, grp, grp_row, channelnumber)
%--------------------------------------------------------------------------
...
% structure array
[~, ser_row, nseries]=getSeriesHeaders(tree, grp_row, grp);
...
for ser=1:nseries
    ...
    for k=1:size(tr_row, 1)
        ...
        tr_s=LocalCheckEntries(tree, tr_row, k);
        ...
        %     
        DataChannels{channelnumber}=data;

...
function calcium_raw_trace_all(results_directory,pathname,x_label,Fs_cal,time)
% only for 1004 !!!!!!!!!!!!!!!!!!!!!!!!!
% new lotos ananlysis
...
% 
    ...
    Time_after=round(2*Fs_cal);  %  after select 2 seconds for caculating the signal 
        ...
        threshold = (max(StimulusData) - mean(StimulusData))*0.5+mean(StimulusData);
        PosAbove=find(StimulusData>threshold);  
        diff_stimu = PosAbove(2:end) - PosAbove(1:end-1);
        
        if PosAbove(1)>=2
           ind_find = [PosAbove(1); PosAbove(find(diff_stimu>1)+1)];
        end
            
        TriggerTime_cal = calclium_stimu_Time(ind_find); 

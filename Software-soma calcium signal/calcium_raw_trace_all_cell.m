...
function calcium_raw_trace_all_cell(results_directory,pathname,x_label,Fs_cal,time,fig_eps)
% only for 1002 !!!!!!!!!!!!!!!!!!!!!!!!!
% new lotos ananlysis
...
for ii = 1:length(txt_files)
    ...
    if length(filename)==7 || length(filename)==8
        ci = ci+1;
...
% 
    ...
    Time_after=round(2*Fs_cal);  %  after select 2 seconds for caculating the signal 
        ...
        TriggerTime_cal = calclium_stimu_Time(ind_find);   %%%%%%%% 0.15可以删去
...
% %     data_peaks_all_files{ii} = mean(data_deltaFf,2);
    ...
    gap=2;
    for cellid=1:size(CalciumData,2)        
        plot(time,CalciumData(:,cellid)+gap*(cellid-1)+2,'k')
        hold on
    end
    hold on;
    ...
    set(gca, 'YTick', 0:10:gap*size(CalciumData,2), 'YTickLabel', 0:5:size(CalciumData,2),'FontSize',10); 
    set(gca,'ylim',[0 gap*(size(CalciumData,2)+2)])
    ylabel('Cell No.#')
    xlabel('Time(s)')
    title(data_filenames{ii}(1:end-4))
    
    saveas(h,fullfile(figure_directory,['calcium transient_allcell_frequency_' data_filenames{ii}(1:end-4)]),'png');
    if fig_eps==1
        print('-depsc2','-painters',fullfile(figure_directory,['calcium transient_allcell_frequency_' data_filenames{ii}(1:end-4) '.eps']));
    end
    close 
end
